...
int MOFSModel::readVars(FILE* file_in, int option4CreateVars) {	
	char var_option, garbage_c;
	char garbage[30];
	fscanf(file_in,"%d",&num_vars);
	printf("numvars:%d\n",num_vars);
	fscanf(file_in,"%c",&garbage_c);
//	fscanf(file_in,"%s",&garbage_c);
 //      fscanf(file_in,"%s",&var_option);
	for (int numVarsCounter = num_vars; numVarsCounter>0; numVarsCounter--) {
//		printf(".i value: %d\n",numVarsCounter);
		VarsNode* new_var;
...
float MOFSModel::inputIrq(float inputs[],float* _output) {
	...
	ROS_INFO("Inputs: %f, %f, %f",inputs[0],inputs[1],inputs[2]);
...
//			ipts = new char[num_inputs];
	...
	/*mostrando las reglas v�lidas*/

	//RuleData *aux_valid_rules = rule_data_list;
	//cout << "numero de inputs: " << rule_store->rule->num_inputs() <<endl;
/*	while (aux_valid_rules != 0) {
		char* sets = aux_valid_rules->inputs_sets;
		for (int i = 0;i<num_inputs;i++) {
			cout << sets[i];
		}
		cout << endl;
		aux_valid_rules = aux_valid_rules->next;
	}*/
//	cin.get();
//	return Action();
}
...
// 				if (aux_rulestore->rule->input0() == 'A') {
// 					Acounter++;
// 				}else if (aux_rulestore->rule->input0() == 'B') {
// 					Bcounter ++;
// 				}else if (aux_rulestore->rule->input0() == 'C') {
// 					Ccounter ++;
// 				}else if (aux_rulestore->rule->input0() == 'D') {
// 					Dcounter ++;
// 				}
			//	cout << "modificando salida" << endl;
				// modifica la salida
...
/*void MOFSModel::guardaPos(float x, float y) {
	xpos_file.open("xpos_file.txt",ios::app);
	xpos_file << x <<endl;
	xpos_file.close();
	ypos_file.open("ypos_file.txt",ios::app);
	ypos_file << y <<endl;
	ypos_file.close();
}*/
...
//			file_out << aux_vars->var->overlapIndex() << endl;
		//	fprintf(save_statistics,"%s\n",aux_vars->var->VName());
		//	aux_vars->var->SalvarIncidencia(save_statistics);
		//	fprintf(save_statistics,"\n");
